...
/* =========================================================
   Enhanced script to move the Similar Tools section below
   the main tool card on mobile/tablet and restore on desktop.
   Works across different layouts with flexible selectors.
   ========================================================= */
(function () {
  const BREAKPOINT = 1024; // screen width in px
  ...
  function getElements() {
    ...
    const similarCard = sideCol.querySelector('.bg-white.rounded-2xl.border.border-gray-200');
    const toolCard = mainCol.querySelector('.bg-white.rounded-2xl.border.border-gray-200');
  ...
  function moveToMobileView() {
    const { mainCol, sideCol, toolCard, similarCard } = getElements();
    if (!mainCol || !sideCol || !toolCard || !similarCard) return;

    // Skip if already moved
    if (similarCard.classList.contains('moved-similar')) return;

    // Insert placeholder to track original location
    if (!placeholder) {
      placeholder = document.createElement('div');
      placeholder.classList.add('similar-placeholder');
      sideCol.insertBefore(placeholder, similarCard);
    }

    similarCard.classList.add('moved-similar');
    similarCard.classList.remove('sticky'); // avoid sticky issues on mobile

    toolCard.insertAdjacentElement('afterend', similarCard);
  }

  function restoreDesktopView() {
    const { sideCol, similarCard } = getElements();
    if (!sideCol || !similarCard) return;

    if (!similarCard.classList.contains('moved-similar')) return;

    if (placeholder && placeholder.parentNode === sideCol) {
      sideCol.insertBefore(similarCard, placeholder.nextSibling);
    } else {
      sideCol.prepend(similarCard); // fallback
    }

    similarCard.classList.remove('moved-similar');
  }
  ...
  window.addEventListener('resize', () => {
    ...
    handleResize.debounce = setTimeout(handleResize, 200);
  });
})();
