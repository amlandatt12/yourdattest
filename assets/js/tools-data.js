...
// MOBILE/TABLET: Move #similar-tools right after #about-tool (<=1024px)
(function () {
  function moveSimilarTools() {
    try {
      const screenWidth = window.innerWidth || document.documentElement.clientWidth;
      const aboutSection = document.querySelector("#about-tool");
      const similarSection = document.querySelector("#similar-tools");
      if (!aboutSection || !similarSection) return;
      // Only apply for mobile/tablet widths
      if (screenWidth <= 1024) {
        // If similarSection is not already after aboutSection, move it
        if (aboutSection.nextElementSibling !== similarSection) {
          aboutSection.insertAdjacentElement("afterend", similarSection);
        }
      } else {
        // For desktop, optionally restore original placement:
        // do nothing so desktop layout remains as-is
      }
    } catch (e) {
      // safe fallback
      console.warn("moveSimilarTools error:", e);
    }
  }

  // Run on load and on resize/orientation change (debounced)
  function debounce(fn, wait) {
    let t;
    return function () {
      clearTimeout(t);
      t = setTimeout(fn, wait || 120);
    };
  }

  document.addEventListener("DOMContentLoaded", moveSimilarTools);
